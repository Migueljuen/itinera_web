...
import { Loader2, Search, X, Users, Tag, CheckCircle2, Circle, ArrowLeft, ArrowRight } from 'lucide-react';
...
const TRAVEL_COMPANIONS = [
    ...
    { id: 'Partner', label: 'Couple', icon: '💕', description: 'Romantic experiences for two' },
    { id: 'Family', label: 'Family', icon: '👨‍👩‍👧‍👦', description: 'Great for families with kids' },
    { id: 'Friends', label: 'Friends', icon: '👥', description: 'Fun group activities' },
    { id: 'Group', label: 'Large Group', icon: '👫', description: 'Suitable for bigger groups' },
    { id: 'Any', label: 'Anyone', icon: '🌟', description: 'Works for all group sizes' },
];

const TagCategoryFilter = ({ categories, activeCategory, onCategoryChange }) => (
    <div className="flex gap-2 mb-4 overflow-x-auto pb-2">
        <button
            onClick={() => onCategoryChange(null)}
            className={`px-4 py-2 rounded-full whitespace-nowrap transition-colors ${activeCategory === null
        ...
        {categories.map((category) => (
            <button
                ...
                className={`px-4 py-2 rounded-full whitespace-nowrap transition-colors ${activeCategory === category.name
                    ? 'bg-blue-600 text-white'
                    : 'bg-gray-100 text-gray-700 hover:bg-gray-200'
                    }`}
            >
...
const CompanionCard = ({ companion, isSelected, onToggle }) => (
    <button
        ...
        className={`relative p-4 rounded-xl border-2 transition-all duration-200 text-left hover:shadow-md ${isSelected
...
const TagButton = ({ tag, isSelected, onToggle }) => (
    <button
        ...
        className={`px-4 py-2 rounded-full border transition-all duration-200 hover:scale-105 ${isSelected
...
const Step3Tags = ({ formData = { tags: [], travel_companions: [] }, setFormData, onNext, onBack }) => {
        ...
        <div className="min-h-screen bg-gray-50">
            ...
            <div className="bg-white border-b border-gray-200 px-4 py-6">
                <div className="max-w-4xl mx-auto">
                    ...
                    <h1 className="text-2xl font-bold text-gray-900 mb-2">Describe Your Experience</h1>
                    <p className="text-gray-600">
                        Help travelers find your experience by selecting relevant tags and ideal group types.
                    </p>
                </div>
            </div>
            ...
            <div className="max-w-4xl mx-auto px-4 py-8">
                ...
                <div className="bg-white rounded-xl p-6 mb-8 shadow-sm">
                    <div className="flex items-center gap-3 mb-4">
                        ...
                        <h2 className="text-xl font-semibold text-gray-900">Who is this perfect for?</h2>
                    </div>
                    <p className="text-gray-600 mb-6">Select all group types that would enjoy this experience.</p>
                    ...
                    {selectedCompanions.length > 0 && (
                        <div className="bg-blue-50 rounded-lg p-4">
                            <p className="text-blue-800 font-medium">
                                ✨ Selected for {selectedCompanions.length} group type{selectedCompanions.length !== 1 ? 's' : ''}
                            </p>
                        </div>
                    )}
                </div>
                ...
                <div className="bg-white rounded-xl p-6 shadow-sm">
                    ...
                    <p className="text-gray-600 mb-6">Choose tags that best describe your experience to help travelers discover it.</p>
                                ...
                                <div className="min-h-[4rem] border-2 border-dashed border-gray-200 rounded-lg p-4">
                                    {selectedTagObjects.length === 0 ? (
                                        <div className="flex items-center justify-center h-16 text-gray-400">
                                            <p>No tags selected yet. Click on tags above to add them.</p>
                                        </div>
                                    ) : (
                                        <div className="flex flex-wrap gap-2">
                ...
                <div className="flex flex-col sm:flex-row gap-4 mt-8">
                    ...
                    <button
                        ...
                        disabled={!canProceed}
                        className={`flex items-center justify-center gap-2 px-6 py-3 rounded-xl font-medium transition-all ${canProceed
                ...
                {(selectedCompanions.length > 0 || selectedTags.length > 0) && (
                    <div className="mt-6 bg-green-50 border border-green-200 rounded-lg p-4">
                        ...
                        <div className="text-sm text-green-700 space-y-1">
                            <p>✓ {selectedCompanions.length} group type{selectedCompanions.length !== 1 ? 's' : ''} selected</p>
                            <p>✓ {selectedTags.length} tag{selectedTags.length !== 1 ? 's' : ''} selected</p>
                            {!canProceed && (
                                <p className="text-amber-700 mt-2">
                                    ⚠️ Please select at least one group type and one tag to continue
                                </p>
                            )}
                        </div>
                    </div>
